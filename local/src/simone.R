...
for (i in seq(1, length(all_rep)-1)) {
  ...
  pdata1 <- data.frame(logfr=logfr_1, fill=rep('CTX', length(logfr_1)),rep=rep(w, length(logfr_1)))
  pdata2 <- data.frame(logfr=logfr_2, fill=rep('NT', length(logfr_1)), rep=rep(w, length(logfr_2)))
  if (i == 1) {
    pdata_all_1 <- pdata1
    pdata_all_2 <- pdata2
  } else {
...
ggsave(histo_f)
# then look for enriched barcodes